defmodule FeelEx.Expression do
  @moduledoc """
  AST representation of a FeelEx's program and AST evaluator.
  """
  require Logger
  ...
  defmodule Access do
    ...
    def new(name, operand) when is_atom(name) and is_struct(operand) do
      %__MODULE__{name: name, operand: operand}
    end
  end
  ...
  defmodule Between do
    ...
    def new(operand, min, max) when is_struct(operand) and is_struct(min) and is_struct(max) do
      %__MODULE__{operand: operand, min: min, max: max}
    end
  end
  ...
  defmodule BinaryOp do
    ...
    def new(type, left_tree, right_tree)
      ...
      %__MODULE__{type: type, left_tree: left_tree, right_tree: right_tree}
    end
  end
  ...
  defmodule Context do
    ...
    def new(keys_with_values) when is_list(keys_with_values) do
      %__MODULE__{keys_with_values: keys_with_values}
    end
  end
  ...
  defmodule FilterList do
    ...
    def new(list, filter) when is_struct(list) and is_struct(filter) do
      %__MODULE__{list: list, filter: filter}
    end
  end
  ...
  defmodule For do
    ...
    def new(iteration_contexts, return_expression) do
      %__MODULE__{iteration_contexts: iteration_contexts, return_expression: return_expression}
    end
  end
  ...
  defmodule List do
    ...
    """
    @spec new([struct()]) :: t()
    def new(elements), do: %__MODULE__{elements: elements}
  end
  ...
  defmodule Negation do
    ...
    """
    @spec new(struct()) :: t()
    def new(operand), do: %__MODULE__{operand: operand}
  end
  ...
  defmodule Quantified do
    ...
    def new(quantifier, list, condition),
      do: %__MODULE__{quantifier: quantifier, list: list, condition: condition}
  end
  ...
  defmodule Range do
    ...
    """
    @spec new(struct(), struct()) :: t()
    def new(first_bound, second_bound),
      do: %__MODULE__{first_bound: first_bound, second_bound: second_bound}
  end
  ...
  def new(:context, expression_list) do
    ...
    new(:expression, Context.new(expression_list))
  end
  ...
  def new(:list, expression_list) do
    ...
    new(:expression, List.new(expression_list))
  end

  def new(:negation, operand) do
    new(:expression, Negation.new(operand))
  end

  def new(:string, string) do
    new(:expression, String_.new(string))
  end
  ...
  def new(:if, {condition_tree, []}, {conditional_statement_tree, []}, {else_statement_tree, []}) do
    %__MODULE__{
      child: %If{
        ...
        conditional_statement: conditional_statement_tree,
